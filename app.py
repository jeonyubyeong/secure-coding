...
def chat():
    ...
    # 예시: 메시지 목록은 DB 대신 임시 데이터로 렌더링
    messages = [
        {'sender': 'user1', 'text': '안녕하세요!', 'timestamp': '10:00'},
        {'sender': user['username'], 'text': '네 안녕하세요!', 'timestamp': '10:01'}
    ]
...
def profile():
    ...
    if request.method == 'POST':
        ...
        if current_pw and new_pw:  # 비밀번호 변경 시도
...
def product_list():
    ...
    cursor.execute("""
        SELECT product.id, product.title, product.description, product.price,
               product.seller_id, user.username
        FROM product
...
def view_product(product_id):
    ...
    # 댓글 가져오기
    cursor.execute("""
        SELECT comment.id, comment.content, comment.user_id, user.username
        FROM comment
    ...
    return render_template('view_product.html', product=product, seller=seller, comments=comments)
...
def add_comment(product_id):
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute("INSERT INTO comment (id, product_id, user_id, content) VALUES (?, ?, ?, ?)",
                   (comment_id, product_id, session['user_id'], content))
    db.commit()
...
def delete_comment(comment_id):
    ...
    # 삭제 권한 확인
    cursor.execute("SELECT * FROM comment WHERE id = ? AND user_id = ?", (comment_id, session['user_id']))
